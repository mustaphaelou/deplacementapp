...
/**
 * Variant B — Region filter pills
 * Combobox with clickable region chips above to pre-filter the list.
...
export function VariantB({ value, onValueChange, error, id }: VariantBProps) {
  const [ selectedRegion, setSelectedRegion ] = useState<string | null>(null)

  const filtered = useMemo(() => {
    if (!selectedRegion) return MOROCCAN_CITIES
    return MOROCCAN_CITIES.filter((c) => c.region === selectedRegion)
  }, [ selectedRegion ])

  const regionsWithCities = useMemo(
    () => REGIONS.filter((r) => filtered.some((c) => c.region === r)),
    [ filtered ]
  )

  const selectedRegionLabel = value ? getCityRegion(value) : undefined
  ...
  return (
    <div className="flex flex-col gap-1.5">
      ...
      <div className="flex flex-wrap gap-1.5">
        ...
        {regionsWithCities.slice(0, 6).map((r) => (
          <button
            key={r}
            type="button"
            onClick={() => {
        ...
        {selectedRegion && (
          <button
            ...
            className="inline-flex items-center gap-0.5 rounded-full px-2 py-0.5 text-xs font-medium text-destructive hover:bg-destructive/10"
          >
            <X className="size-3" />
            Effacer filtre
          </button>
      ...
      >
        ...
        >
          <div className="flex items-center gap-2 truncate">
            ...
            <ComboboxPrimitive.Value placeholder="Rechercher une ville..." />
          </div>
                  ...
                  ? filtered
                    ...
                    .map((city) => (
                      <ComboboxPrimitive.Item
                        key={city.name}
                        value={city.name}
                        className="relative flex w-full cursor-default items-center rounded-sm py-1.5 pl-8 pr-2 text-sm outline-none data-[highlighted]:bg-accent data-[highlighted]:text-accent-foreground"
                      >
      ...
      {selectedRegion && (
        <p className="text-xs text-muted-foreground">
          Filtré par région : {selectedRegion}
        </p>
      )}
    </div>
  )
