...
- **Malformed tags** (`v-weird`, not SemVer): no versioned images and no GitHub Release — the `docker/metadata-action` semver patterns filter the image side and the strict `vX.Y.Z` gate (`release-gate` step) blocks the Release object. Only the full-sha image is still pushed (harmless); `latest` and the versioned tags are left untouched.
