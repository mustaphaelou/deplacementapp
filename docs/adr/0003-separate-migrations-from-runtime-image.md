...
- The migrator image is published for `linux/amd64` only — it runs solely as a one-shot `migrate` compose service on an amd64 Coolify host, and no consumer pulls an arm64 migrator. Every run therefore avoids paying QEMU cost for that platform; the runner image keeps building `linux/amd64,linux/arm64`. The arm64 drop is this consequence's direct outcome.
