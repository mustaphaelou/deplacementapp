...
- **Trigger scope:** the `deploy` job depends on `build-and-publish` (verify transitively covered) and is gated at job level on `build-and-publish.outputs.is_release` — from the `release-gate` step, the single owner of release-ness — being `true`. Branch pushes, pull requests, and tag-less dispatches deploy nothing. The sole non-Release path in is a `workflow_dispatch` run with the `deploy-dry-run` input, which exercises the job in dry-run mode without firing.
