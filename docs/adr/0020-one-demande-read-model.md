...
- **Retired with the decision.** The decision-agnostic `findByEtapes` (production-orphaned by the queue swap — a stage read states its decision through `findPendingByEtapes`) and the `rollup` view in `PIPELINE_VIEWS` (the three-of-five-lanes set behind the old « Total »; `rollupEtapes` goes with it) are gone — no surface can re-adopt the old shapes.
