...
- Future architecture reviews should not suggest re-expanding the image map into per-image blocks — the matrix `include:` row is the single data-rot seam (one added row = a third image), and the twin build/push blocks this ticket replaced are gone. The twin-block deepening candidate is closed.
